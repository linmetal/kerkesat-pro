...
import ARCS from './cards.arcs';
import INTERRUPTS from './cards.interrupts';
import { tierIndex, rand } from './engine';

// Arcs with a weight can be drawn like any other scene; the rest only ever
// arrive because an earlier decision scheduled them.
export const DECK = [...EARLY, ...MID, ...LATE, ...ARCS.filter((c) => c.weight > 0)];
export const ALL_CARDS = [...DECK, ...INTERRUPTS, ...ARCS.filter((c) => !c.weight)];

export const now = (s) => (s.year - 1) * 4 + s.quarter;
...
export function nextCard(s) {
  // A sequel a past decision scheduled outrules everything, once it is due.
  const due = (s.queued || [])
    .filter((q) => q.at <= now(s))
    .map((q) => cardById(q.id))
    .filter((c) => c && (!c.req || c.req(s)));
