...
export function applyFx(s, fx) {
  ...
  // People join, drift and leave through the same effect object as everything else.
  if (!s.people) s.people = {};
  if (fx.join) {
    Object.keys(fx.join).forEach((role) => {
      if (!s.people[role]) s.people[role] = makePerson(s, role, fx.join[role] === true ? {} : fx.join[role]);
    });
  }
  if (fx.loyalty) {
    Object.keys(fx.loyalty).forEach((role) => {
      if (s.people[role]) s.people[role].loyalty = clamp(s.people[role].loyalty + fx.loyalty[role], 0, 100);
    });
  }
  // Someone who walks out of your company can walk into the market as a rival.
  if (fx.spawnRivalFrom && s.people[fx.spawnRivalFrom]) {
    const p = s.people[fx.spawnRivalFrom];
    s.rivalSeq = (s.rivalSeq || 0) + 1;
    s.rivals.push({
      id: s.rivalSeq,
      name: `${p.name.split(' ').pop()} & Co.`,
      ceo: p.name,
      trait: 'aggressive',
      traitName: { sq: 'agresiv', en: 'aggressive' },
      growth: 0.06,
      aggro: 0.95,
      value: Math.max(60000, companyValue(s) * 0.28),
      alive: true,
    });
  }
  if (fx.leave) fx.leave.forEach((role) => { delete s.people[role]; });
}
...
export function newGame({ name, industry, seed }) {
  ...
  s.rngState = s.seed;
  s.yearStats = { year: 1, startRevenue: 0, startNW: netWorth(s), profit: 0 };
...
function makeRival(s, i) {
  ...
  s.rivalSeq = (s.rivalSeq || 0) + 1;
  const ceo = makePerson(s, 'rivalCeo');
  return {
    id: s.rivalSeq,
...
export function advanceQuarter(s) {
  ...
  if (s.quarter > 4) {
    ...
    const st = s.yearStats;
    s.yearReport = {
      year: st.year,
      revenue: s.revenue,
      revenueGrowth: st.startRevenue > 0 ? (s.revenue - st.startRevenue) / st.startRevenue : null,
      profit: st.profit,
      nwGain: nwNow - st.startNW,
      teamSize: s.teamSize,
      rivals: s.rivals.filter((r) => r.alive).length,
      economy: s.economy.phase,
    };
    s.yearStats = { year: s.year, startRevenue: s.revenue, startNW: nwNow, profit: 0 };
  }
